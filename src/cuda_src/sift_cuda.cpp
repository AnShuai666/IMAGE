/*
 * @功能      sift源文件副本及cuda实现(有错误,暂不实现)
 * @姓名      杨丰拓
 * @日期      2019-5-20
 * @时间      9:41
*/
#include <fstream>
#include "cuda_include/sift_cuda.hpp"
...
IMAGE_NAMESPACE_BEGIN
    ...
    image::Sift_CUDA::Sift_CUDA(image::Sift_CUDA::Options const &options):
    ...
    void
    image::Sift_CUDA::set_image(image::ByteImage::ConstPtr img)
    {
    ...
    image::Sift_CUDA::process()
    {
        ...
        clog<<"在第"<<__LINE__<<"行退出"<<endl;
        exit(0);
        if (this->options.debug_output)
        {
            std::cout << "Sift_CUDA: 检测到 "
                      << this->keypoints.size()
                      << " 个关键点, 用时 "
                      << timer.get_elapsed()
                      << "毫秒。"
        ...
        if (this->options.debug_output)
        {
            std::cout << "Sift_CUDA: 正在定位与过滤关键点中......"<<std::endl;
        }
        ...
        if (this->options.verbose_output)
        {
            std::cout << "Sift_CUDA: 正在生成描述子列表......"<<std::endl;
        }
    ...
    void
    image::Sift_CUDA::load_lowe_descriptors(std::string const &filename, image::Sift_CUDA::Descriptors *result)
    {
        ...
        {
            Sift_CUDA::Descriptor descriptor;
    ...
    void
    image::Sift_CUDA::set_float_image(image::FloatImage::ConstPtr img)
    {
        if (img->channels() != 1 && img->channels() != 3)
        {
            throw std::invalid_argument("需要灰度图或者彩色图");
    ...
    void
    image::Sift_CUDA::create_octaves(void)
    {
        this->octaves.clear();
    ...
    void
    image::Sift_CUDA::add_octave(image::FloatImage::ConstPtr image,float has_sigma, float target_sigma)
    {
    ...
    void
    image::Sift_CUDA::add_octave2(image::FloatImage::ConstPtr image,float has_sigma2, float target_sigma2)
    {
        //图像固有尺度为has_sigma,要模糊到尺度为target_sigma,连续模糊有如下性质:
        //L * G(sigma1) * G(sigma2) = L * G (sart(sigma1^2 + sigma^2))
        //即 L * G(has_sigma) * G(sigma) = L * G(target_sigma),
    ...
    void
    image::Sift_CUDA::extrema_detection()
    {
        ...
        {
            image::Sift_CUDA::Octave const& octave(this->octaves[i]);
    ...
    void image::Sift_CUDA::extrema_detection(image::Image<float>::ConstPtr *s, int octave_index, int sample_index)
    {
        ...
        //cout<<"**********************"<<endl;
        cout<<"cpu:"<<sum<<endl;
        extrema_detection_cu(s[1]->width(),s[1]->height(),noff,octave_index,sample_index,
                &s[0]->at(0),&s[1]->at(0),&s[2]->at(0));
    }

    void
    image::Sift_CUDA::keypoint_localization()
    {
        int num_sigular = 0;
        int num_keypoints = 0;
        ...
        for (int i = 0; i < this->keypoints.size(); ++i)
        {
            ...
            image::Sift_CUDA::Octave const& octave(this->octaves[kp.octave - this->options.min_octave]);
    ...
    void
    image::Sift_CUDA::descriptor_generation()
    {
    ...
    void
    image::Sift_CUDA::generate_grad_ori_images(image::Sift_CUDA::Octave *octave)
    {
        octave->img_grad.clear();
    ...
    void
    image::Sift_CUDA::orientation_assignment(const image::Sift_CUDA::Keypoint &kp, const image::Sift_CUDA::Octave *octave,std::vector<float> &orientations)
    {
        //直方图柱状个数
        int const nbins = 36;
    ...
    bool image::Sift_CUDA::descriptor_assignment(const image::Sift_CUDA::Keypoint &kp, image::Sift_CUDA::Descriptor &desc,const image::Sift_CUDA::Octave *octave)
    {
    ...
    float
    image::Sift_CUDA::keypoint_relative_scale(const image::Sift_CUDA::Keypoint &kp)
    {
        return this->options.base_blur_sigma * std::pow(2.0f,(kp.sample + 1.0f) / this->options.num_samples_per_octave);
    }

    float
    image::Sift_CUDA::keypoint_absolute_scale(const image::Sift_CUDA::Keypoint &kp)
    {
